...
resource "aws_secretsmanager_secret" "mysql_secrets_manager" {
  ...
  name        = "std17/mysql/secrets" # 콘솔에 표시될 이름
}
# 2. JSON형식으로 보안암호 지정
...
resource "aws_serverlessapplicationrepository_cloudformation_stack" "mysql_rotation" {
  ...
  parameters = {
    ...
    vpcSecurityGroupIds = aws_security_group.std17_lambda_sg.id
  }
}
...
#   depends_on = [
#     aws_db_instance.mysql # 또는 aws_rds_cluster 등 실제 RDS 리소스 이름
#   ]
