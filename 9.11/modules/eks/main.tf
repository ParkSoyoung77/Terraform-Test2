...
# ======================================================
# EKS 클러스터(컨트롤플레인)용
resource "aws_security_group" "std17_cluster_sg" {
...
# cluster_sg 인바운드: eks-node-sg, 0.0.0.0/0
resource "aws_security_group_rule" "cluster_sg_from_node" {
    type                     = "ingress"
    from_port                = 443
    to_port                  = 443
    protocol                 = "tcp"
    security_group_id        = aws_security_group.std17_cluster_sg.id
    source_security_group_id = aws_security_group.std17_eks_node_sg.id
}

resource "aws_security_group_rule" "cluster_sg_from_internet" {
    type              = "ingress"
    from_port         = 443
    to_port           = 443
    protocol          = "tcp"
...
# eks-node-sg 인바운드: external-alb-sg(80,443), cluster_sg(10250), self(all)
resource "aws_security_group_rule" "node_sg_http_from_alb" {
    type                     = "ingress"
    from_port                = 80
    to_port                  = 80
    protocol                 = "tcp"
...
resource "aws_security_group_rule" "node_sg_https_from_alb" {
    type                     = "ingress"
    from_port                = 443
    to_port                  = 443
    protocol                 = "tcp"
...
resource "aws_security_group_rule" "node_sg_kubelet_from_cluster" {
    type                     = "ingress"
    from_port                = 10250
    to_port                  = 10250
    protocol                 = "tcp"
    security_group_id        = aws_security_group.std17_eks_node_sg.id
    source_security_group_id = aws_security_group.std17_cluster_sg.id
}

resource "aws_security_group_rule" "node_sg_self_all" {
    type              = "ingress"
    from_port         = 0
    to_port           = 0
    protocol          = "-1"
...
resource "aws_launch_template" "launch_template" {
    ...
    vpc_security_group_ids = [
        ...
        aws_security_group.std17_cluster_sg.id,
